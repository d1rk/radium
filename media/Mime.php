...
class Mime {
	...
	/**
	 * returns correct mime-type according to extension
	 *
	 * @param string $extension file-extension to retrieve mime-type for
	 * @return string|array the corresponding mime-type
	 */
	public static function type($extension = null) {
		if(!$extension) {
			return 'application/octet-stream';
		}
		static::init();

		return (isset(static::$mime_types[$extension]))
			? static::$mime_types[$extension]
	 ...
	 *
	 * @param string $extension file-extension to retrieve mime-type for
	 * @return array the corresponding mime-type
	 */
	public static function types($extension = null) {
		return static::type($extension);
	}

	/**
	 * will print out headers for given extension
	 *
	 * @param string $extension file-extension to render header for
	 * @param array $options additional options
	 *        - `download`: if given, will provide file for download with given name
	 *        - `cache`: if given, will provide caching headers accordingly, defaults to
	 *                   `no-cache, must-revalidate`
	 * @return void Attention: this will set headers instead of returning something
	 */
	public static function header($extension, array $options = array()) {
		$defaults = array(
			'download' => false,
		...
		header(sprintf('Content-Type: %s', static::type($extension)));
		if ($options['cache']) {
	 ...
	 *
	 * @param array $types if given, will initialize types as current mime-types
	 * @return array the corresponding mime-type
	 */
	public static function init(array $types = array()) {
		if (empty(static::$mime_types)) {
			static::$mime_types = require __DIR__ . '/mime.php';
		}
		if (!empty($types)) {
			static::$mime_types = $types;
		}
		return;
	}
